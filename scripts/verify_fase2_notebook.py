...
NOTEBOOK = ROOT / "notebooks" / "fase2.ipynb"
FIGURES = ROOT / "results" / "figures" / "02_preprocessing"
PHASE1_TABLES = ROOT / "results" / "tables" / "01_raw_eda"
PHASE2_TABLES = ROOT / "results" / "tables" / "02_preprocessing"
...
def section_intro_checks(cells) -> list[dict[str, object]]:
    ...
    for cell in cells:
        ...
        mentions_method = bool(
            re.search(
                r"target|IQR|Spearman|cardinalidad|identificador|one-hot|escalador|nulos|duplicados|train|QFS|PAPER_QFS|umbral|codifica|recarga|estructural|snake_case|renombrado|tipo",
                intro,
                flags=re.IGNORECASE,
            )
        )
...
def displayed_pngs(cells) -> set[str]:
    ...
    for name in literal_pattern.findall(source):
        if "{" not in name:
            displayed.add(f"results/figures/02_preprocessing/{name}")
    if "fase2_outliers_iqr_{dataset_name}.png" in source:
        for dataset in DATASETS:
            displayed.add(f"results/figures/02_preprocessing/fase2_outliers_iqr_{dataset}.png")
    if "fase2_distribuciones_numericas_{nombre_dataset}.png" in source:
        for dataset in DATASETS:
            displayed.add(f"results/figures/02_preprocessing/fase2_distribuciones_numericas_{dataset}.png")
    for name in [
    ...
    categories_path = PHASE2_TABLES / "fase2_categorias_predictoras.csv"
    if categories_path.exists() and "fase2_categorica_{dataset_name}_" in source:
        categories = pd.read_csv(categories_path)
        for row in categories[["dataset", "variable"]].drop_duplicates().itertuples(index=False):
            variable_name = nombre_archivo_seguro(row.variable)
            displayed.add(f"results/figures/02_preprocessing/fase2_categorica_{row.dataset}_{variable_name}.png")
    return displayed
...
def phase1_preconditions() -> list[str]:
    return [name for name in PHASE1_REQUIRED_TABLES if not (PHASE1_TABLES / name).exists()]
...
def main() -> None:
    ...
    report = {
        "phase1_missing_tables": phase1_preconditions(),
    ...
    report["checks"] = {
        "phase1_preconditions_available": not report["phase1_missing_tables"],
