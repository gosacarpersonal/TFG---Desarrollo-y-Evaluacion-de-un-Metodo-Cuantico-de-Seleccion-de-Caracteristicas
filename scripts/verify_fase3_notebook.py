...
NOTEBOOK = ROOT / "notebooks" / "fase3.ipynb"
FIGURES = ROOT / "results" / "figures" / "03_postprocessing_audit"
...
PHASE2_REQUIRED_TABLES = [
    "fase2_codificacion_target.csv",
    "fase2_renombrado_columnas.csv",
    "fase2_dimensionalidad_encoding.csv",
]
...
def section_intro_checks(cells) -> list[dict[str, object]]:
    ...
    for cell in cells:
        ...
        mentions_method = bool(
            re.search(
                r"target|IQR|Spearman|cardinalidad|identificador|one-hot|encoder|escalador|nulos|duplicados|split|QFS|PAPER_QFS|umbral|codifica|recarga|estructural|renombrado|tipo|PCA|VIF|Fisher|Wasserstein|redundancia|dimensionalidad",
                intro,
                flags=re.IGNORECASE,
            )
        )
...
def displayed_pngs(cells) -> set[str]:
    ...
    literal_pattern = re.compile(r"cerrar_figura_narrativa\([^,]+,\s*[f]?[\"']([^\"']+\.png)[\"']\)")
    for name in literal_pattern.findall(source):
        if "{" not in name:
            displayed.add(f"results/figures/03_postprocessing_audit/{name}")
    for pattern in [
        "fase3_target_conservacion_{nombre_dataset}.png",
        "fase3_distribucion_conservacion_{nombre_dataset}.png",
        "fase3_asociacion_{nombre_dataset}.png",
        "fase3_redundancia_{nombre_dataset}.png",
    ]:
        if pattern in source:
            prefix, suffix = pattern.split("{nombre_dataset}")
            for dataset in DATASETS:
                displayed.add(f"results/figures/03_postprocessing_audit/{prefix}{dataset}{suffix}")
    return displayed
...
def internal_audit_hits(cells) -> dict[str, list[int]]:
    ...
    for index, cell in enumerate(cells, start=1):
        ...
        for word in INTERNAL_AUDIT_WORDS:
            if word == "audit":
                pattern = r"(?<![A-Za-zÁÉÍÓÚÜÑáéíóúüñ])audit(?![A-Za-zÁÉÍÓÚÜÑáéíóúüñ])"
            else:
...
def phase2_preconditions() -> list[str]:
    return [name for name in PHASE2_REQUIRED_TABLES if not (PHASE2_TABLES / name).exists()]
...
def main() -> None:
    ...
    report = {
        "phase2_missing_tables": phase2_preconditions(),
    ...
    report["checks"] = {
        "phase2_preconditions_available": not report["phase2_missing_tables"],
